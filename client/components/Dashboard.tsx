...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const addChart = (query: string, name: string, topic: string | undefined) => {
    ...
    setCoreData((prev: ChartDataType[]): ChartDataType[] => [
      ...prev,
      newChart,
    ]);
  };
    ...
    const getMetrics = async () => {
      try {
        const response = await axios.get(
          `http://${server}/api/v1/label/__name__/values`
        );
  ...
  return (
    <>
      <div className="flex h-screen w-auto">
        {/*POPUP WONT RETURN ANYTHING AT FIRST RETURN IS EMPTY IF FALSE*/}
        <div className="w-screen flex flex-col">
          <div className="flex items-center justify-center content-center px-4 bg-slate-900">
            ...
            {/* <button className="btn btn-primary" onClick={(e) => handleAlertPopup(e)}>
              View Alerts
            </button> */}
